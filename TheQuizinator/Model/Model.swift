...
class QuestionValues {
    ...
    func verifyRoundContinuation() -> Bool {
    ...
    func retrieveQuestionValues(index: Int) -> [String:String] {
        
        let selectedQuestionDict = Questions.init().pool[index]
    ...
    func checkTheAnswer(buttonInput: Int) -> String {
        ...
        QuestionValues.init().questionsAsked += 1
   
        let answerDataPull = retrieveQuestionValues(index: indexOfSelectedQuestion)
        let correctAnswer = answerDataPull["Answer"]
        
        if (buttonInput == 1 && correctAnswer == "1") {
            correctQuestions += 1
            return "Correct!"
            
        } else if (buttonInput == 2 && correctAnswer == "2") {
            correctQuestions += 1
            return "Correct!"
            
        } else if (buttonInput == 3 && correctAnswer == "3") {
            correctQuestions += 1
            return "Correct!"
            
        } else if (buttonInput == 4 && correctAnswer == "4") {
            correctQuestions += 1
            return "Correct!"
        } else {
            return "Incorrect!"
